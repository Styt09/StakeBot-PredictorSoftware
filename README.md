...
## Critical Hardening Phase — Live-Order Hazard Removed and Zerodha Shadow Readiness Gate

The institutional audit produced a **NO-GO** for real Zerodha shadow trading because the repository still had a latent live-broker routing branch, env-string-only Zerodha auth validation, smoke-level shadow scripts, and evidence sections without mandatory provenance. This hardening phase closes those specific blockers without adding trading features, without enabling `LIVE_AUTO`, and without placing real orders.

### Live-order hazard removal

`AlphaGateXSettings.live_orders_enabled` is now permanently `false`, `LIVE_AUTO` raises `RealOrderPathForbidden`, and `OrderRouter` rejects any supplied live broker adapter before routing can occur. The only permitted routing path is paper/preview evidence. A static scan (`python scripts/no_live_order_static_scan.py`) is part of `make check` and CI so accidental broker order calls are detected before merge.

### Zerodha read-only validation

`ZerodhaAuthService` now requires a `ZerodhaProfileClient` abstraction that can call only a read-only profile endpoint. Validation requires API key, access token, profile reachability, non-revoked session status, and optional expected-user matching. If the profile client is missing, unreachable, revoked, or mismatched, validation returns `ZERODHA_UNAVAILABLE`; it never calls order endpoints.

### Zerodha shadow feed runner prerequisites

`ZerodhaShadowFeedRunner` is a read-only runner skeleton for an operator-provided KiteTicker integration. It resolves/subscribes instrument tokens, tracks heartbeat freshness, rejects malformed and duplicate ticks, emits stale-feed events, supports reconnect/backoff bookkeeping, and shuts down gracefully. It does not create orders, approvals, or broker mutations.

### Shadow readiness checklist

`python scripts/run_shadow_day.py` is now a readiness gate, not a shadow launcher. It exits with `NO_GO_SHADOW_READINESS` unless config is valid, persistence is healthy, Zerodha read-only profile validation succeeds, the live-order forbidden guard passes, and an operator-provided read-only feed runner integration exists. Real Zerodha shadow trading must not start from this repository until every checklist item is satisfied.

### Evidence provenance requirement

Evidence-pack sections now carry provenance metadata: `source_store`, `event_id_range`, `snapshot_id`, `audit_chain_hash`, `generated_at`, and `verification_status`. Missing provenance is exported as `DATA_UNAVAILABLE`, and final certification fails when required provenance is absent. This does not make evidence tamper-proof; it makes missing or unverified provenance explicit.

### Remaining blockers before real shadow

Real Zerodha shadow trading still requires a production KiteTicker wrapper, token refresh operations, durable production storage or hardened SQLite operations, external monitoring/alerting, replay/soak tests, and human operational sign-off. `go_live_allowed` remains `false` and the maximum recommendation ceiling remains manual review/certification only.

## Zerodha Read-Only Profile Smoke Test

This repository now includes a **one-shot read-only Zerodha profile/session smoke test**. It is not a WebSocket runner, not a shadow-trading launcher, not an order-preview flow, and not a trading-signal workflow.
...
export ZERODHA_EXPECTED_USER_ID="..."   # strongly recommended
export AUDIT_DB_PATH="./alpha_gate_x_audit.db"
```
...
- `PROFILE_SMOKE_GO` means the read-only profile endpoint was reachable, credentials were accepted, the session was not marked revoked, and the returned user id matched `ZERODHA_EXPECTED_USER_ID` when configured.
- `PROFILE_SMOKE_NO_GO` means credentials, network, auth, session status, or expected-user validation failed closed.

The script persists a redacted audit event: `ZerodhaReadOnlyProfileSmokePassed` or `ZerodhaReadOnlyProfileSmokeFailed`. It never starts WebSocket shadow trading, never generates signals, never creates order previews, never imports an order wrapper, and never calls a broker order API.

### Next step after pass

A passing profile smoke test allows only the next operational checkpoint: a **read-only WebSocket connection smoke test** with no signal generation and no shadow trading. Real shadow trading remains blocked until feed freshness, persistence, reconciliation, monitoring, and evidence-provenance checks pass.

## Zerodha Integration Audit Script

Use the integration audit after the profile smoke test to validate repository-side readiness for the Zerodha modules without starting WebSocket shadow trading:
...
The audit checks imports for `zerodha_auth.py`, `zerodha_market_data.py`, `zerodha_instruments.py`, and `zerodha_instrument_manager.py`; verifies pytest/dev dependency availability; loads required environment variables from the shell or `.env`; runs the read-only profile smoke test; validates a configured instrument dump if `ZERODHA_INSTRUMENT_DUMP_PATH` is set; and performs a synthetic market-data payload mapping/rejection check. A `FAIL` report is a hard block for WebSocket smoke testing. The audit does not start WebSockets, generate signals, create previews, or place orders.
