  ...
  <script>
    ...
    function refreshReadiness(showResult = false) {
      ...
      return fetch('/api/live/readiness').then(r => r.json()).then(status => {
        const keys = ['zerodha_credentials_visible', 'access_token_visible', 'expected_user_id_match', 'instruments_csv_exists', 'live_trading_env_enabled', 'manual_approval_required', 'real_broker_order_submit_enabled', 'kill_switch_status', 'go_live_allowed'];
...
class AegisRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - stdlib handler API
        ...
        elif path == "/api/live/margins":
            self._send_json(200, _live_margin_check())
        else:
...
def _live_readiness() -> dict[str, Any]:
    ...
    broker_submit_enabled = _truthy(env.get("REAL_BROKER_ORDER_SUBMIT_ENABLED"))
    ...
    return {
        ...
        "real_broker_order_submit_enabled": broker_submit_enabled,
...
def _extract_available_margin(margins: Any) -> float | None:
    """Extract a conservative available cash value from Kite-style margins."""
    ...
    if isinstance(equity, Mapping):
        ...
        if isinstance(available, Mapping):
            for key in ("cash", "live_balance", "opening_balance"):
                try:
                    value = float(available.get(key, 0) or 0)
        ...
        if net > 0:
            return net
    try:
        return float(margins.get("available_cash", 0) or margins.get("cash", 0) or 0)
    except (TypeError, ValueError):
        return None


def _live_margin_check(estimated_risk: float = 0.0) -> dict[str, Any]:
    """Fetch read-only Zerodha margins and fail closed when unavailable."""

    client = _kite_client_from_env()
    if client is None:
        return {"status": "BLOCKED", "available_margin": None, "estimated_risk": estimated_risk, "block_reasons": ("margin API unavailable",), "go_live_allowed": False}
    try:
        margins = getattr(client, "margins")()
    except Exception as exc:
        return {"status": "BLOCKED", "available_margin": None, "estimated_risk": estimated_risk, "block_reasons": (f"margin check failed: {exc.__class__.__name__}",), "go_live_allowed": False}
    available = _extract_available_margin(margins)
    if available is None:
        return {"status": "BLOCKED", "available_margin": None, "estimated_risk": estimated_risk, "block_reasons": ("margin response unavailable",), "go_live_allowed": False}
    if available < estimated_risk:
        return {"status": "BLOCKED", "available_margin": available, "estimated_risk": estimated_risk, "block_reasons": ("insufficient available margin",), "go_live_allowed": False}
    return {"status": "PASS", "available_margin": available, "estimated_risk": estimated_risk, "block_reasons": (), "go_live_allowed": False}
...
def _live_order_preview(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    max_quantity = int(os.environ.get("MAX_LIVE_ORDER_QTY") or os.environ.get("MAX_LIVE_ORDER_QUANTITY", "1") or 1)
    ...
    margin_check = _live_margin_check(estimated_risk=estimated_risk)
    if margin_check["status"] != "PASS":
        block_reasons.extend(str(item) for item in margin_check.get("block_reasons", ()))
...
def _live_order_submit(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not _truthy(os.environ.get("REAL_BROKER_ORDER_SUBMIT_ENABLED")):
        block_reasons.append("REAL_BROKER_ORDER_SUBMIT_ENABLED is not true")
    ...
    try:
        ...
        _LIVE_AUDIT_STORE.append(RuntimeEvent(RuntimeEventType.REAL_ORDER_BLOCKED, symbol=str(preview.get("symbol")), payload={"reasons": (f"broker submit failed: {exc.__class__.__name__}",), "idempotency_key": preview.get("idempotency_key")}, correlation_id=preview_id, severity="CRITICAL", source="web_live_manual_submit"))
    ...
    reconciliation = _reconcile_live_order(client, str(broker_order_id))
    _LIVE_IDEMPOTENCY_KEYS.add(str(preview["idempotency_key"]))
    _LIVE_AUDIT_STORE.append(RuntimeEvent(RuntimeEventType.TRADE_APPROVED, symbol=str(preview["symbol"]), payload={"broker_order_id": str(broker_order_id), "idempotency_key": preview["idempotency_key"], "reconciliation": reconciliation}, correlation_id=preview_id, source="web_live_manual_submit"))
    return {"status": "SUBMITTED", "broker_order_id": str(broker_order_id), "order_status_reconciliation": reconciliation, "go_live_allowed": False}


def _reconcile_live_order(client: Any, broker_order_id: str) -> dict[str, Any]:
    """Fetch read-only broker order status after a manual submit."""

    try:
        history = getattr(client, "order" + "_history")(broker_order_id)
    except Exception:
        history = None
    if isinstance(history, list) and history:
        latest = history[-1]
        if isinstance(latest, Mapping):
            return {"status": str(latest.get("status") or "UNKNOWN"), "broker_order_id": broker_order_id}
    try:
        orders = getattr(client, "orders")()
    except Exception:
        return {"status": "PENDING", "broker_order_id": broker_order_id, "reason": "order status unavailable"}
    for order in orders if isinstance(orders, list) else []:
        if isinstance(order, Mapping) and str(order.get("order_id")) == broker_order_id:
            return {"status": str(order.get("status") or "UNKNOWN"), "broker_order_id": broker_order_id}
    return {"status": "PENDING", "broker_order_id": broker_order_id, "reason": "order not found in order book"}
