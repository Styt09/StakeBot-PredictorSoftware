...
def test_risk_gate_blocks_weekly_monthly_and_concentration_limits() -> None:
    gate = RiskGate(
        RiskConfig(
            capital=100_000,
            max_daily_loss_percent=1.0,
            max_weekly_loss_percent=2.0,
            max_monthly_loss_percent=4.0,
            max_symbol_exposure_percent=10.0,
    ...
    risk = gate.assess(
        ...
        context=RiskContext(
            average_volume=25_000,
            weekly_realized_loss=2_000,
            monthly_realized_loss=4_000,
            symbol_exposure_value=10_000,
            sector_exposure_value=20_000,
        ),
    )

    assert risk.status == RiskStatus.FAIL
    assert "weekly loss limit hit" in risk.reasons
    assert "monthly loss limit hit" in risk.reasons
    assert "symbol concentration limit hit" in risk.reasons
    assert "sector concentration limit hit" in risk.reasons
