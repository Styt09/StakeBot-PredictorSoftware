...
def _base_pack(tmp_path):
    ...
    provenance = pack.sections["audit_events_json"]["provenance"]
    pack.sections["validation_report_json"] = {"go_live_allowed": False, "recommendation": "CONTINUE_SHADOW", "provenance": provenance}
    pack.sections["runtime_snapshot_json"] = {"mode": "PAPER_TRADING", "go_live_allowed": False, "provenance": provenance}
    pack.sections["research_ml_json"] = {"recommendation": "CONTINUE_RESEARCH", "go_live_allowed": False, "provenance": provenance}
    pack.sections["robustness_validation_json"] = {"scorecard": RobustnessScorecard(85, 90, 100, 80, 80, 80, 90, 100, 90, RobustnessRecommendation.READY_FOR_MANUAL_REVIEW, (), (), (), False).to_dict(), "provenance": provenance}
    pack.sections["multi_strategy_json"] = {"allocations": {"allocations": {"A": 0.5, "B": 0.5}}, "health_scores": {"A": {"status": "HEALTHY"}, "B": {"status": "HEALTHY"}}, "overlap_warnings": (), "provenance": provenance}
    pack.sections["options_risk_json"] = {"greeks": {"data_status": "OK"}, "iv_metrics": {"data_status": "OK"}, "expiry_metrics": {"data_status": "OK"}, "concentration_metrics": {"approved": True}, "risk_warnings": (), "go_live_allowed": False, "provenance": provenance}
    pack.sections["execution_realism_json"] = {"execution_reports": ({"fill_ratio": 1.0},), "latency_reports": (), "fill_ratio": 1.0, "warnings": (), "impact_warnings": (), "go_live_allowed": False, "provenance": provenance}
    return pack, shadow
...
def test_certification_pass_warning_fail_and_scorecard(tmp_path) -> None:
    ...
    provenance = pack.sections["audit_events_json"]["provenance"]
    warning = framework.certify_area(next(area for area in framework.AREA_EVIDENCE if area.value == "Options Risk"), {**pack.sections, "options_risk_json": {"warnings": ("minor warning",), "go_live_allowed": False, "provenance": provenance}})
    assert warning.status == CertificationStatus.PASS_WITH_WARNINGS
    failed = framework.certify_area(next(area for area in framework.AREA_EVIDENCE if area.value == "HA/DR"), {**pack.sections, "ha_disaster_recovery_json": {"go_live_allowed": True, "provenance": provenance}})
