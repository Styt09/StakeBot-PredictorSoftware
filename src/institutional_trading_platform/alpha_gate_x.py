...
paper/live mode controls, duplicate-order prevention, and an auditable order
state machine.  Live order placement is deliberately abstracted behind a broker
protocol and is blocked unless both ``TRADING_MODE=LIVE_AUTO`` and
...
class AlphaGateXSettings:
    ...
    def live_orders_enabled(self) -> bool:
        ...
        return self.trading_mode == TradingMode.LIVE_AUTO and self.live_trading

    def validate_live_trading(self) -> None:
        """Reject unsafe live-trading configuration."""

        if self.trading_mode == TradingMode.LIVE_AUTO and not self.live_trading:
            raise ValueError("LIVE_AUTO requires LIVE_TRADING=true")
...
class RiskGate:
    ...
    def assess(self, entry: float, stop_loss: float, context: RiskContext) -> RiskDecision:
        ...
        reasons: list[str] = []
        max_loss = self.config.capital * (self.config.max_daily_loss_percent / 100.0)
        ...
        if context.daily_realized_loss >= max_loss:
...
class OrderRouter:
    """Route approved ALPHA-GATE X signals to paper or live broker adapters."""
    ...
    def route(self, signal: AlphaGateXSignal, risk_decision: RiskDecision, manual_approved: bool = False) -> OrderResult:
        ...
        if not self.settings.live_orders_enabled:
            return OrderResult(signal.correlation_id, OrderState.ORDER_REJECTED, rejection_reason="live trading blocked by configuration")
        if self.live_broker is None or not hasattr(self.live_broker, "place_order"):
            return OrderResult(signal.correlation_id, OrderState.ORDER_REJECTED, rejection_reason="live broker adapter unavailable")
        return self.live_broker.place_order(request)
