...
def _orchestrator(tmp_path, *, config_valid: bool = True, recon_pass: bool = True):
    ...
    return ShadowTradingOrchestrator(config, store, bus, engine, ZerodhaAuthService(ZerodhaAuthConfig("key", "token"), event_bus=bus), ZerodhaInstrumentManager(event_bus=bus), recon, CrashRecoveryService(store, event_bus=bus), DashboardSummaryService(store), ShadowRunValidator(store)), store
